...
# by CSI #######################################
    def BestTimeconstant(self, feq, minperiod=5):
        """
        find the closed available timeconstant for a given value
        """

        val = (1/float(feq))*minperiod
...
#        key = options[min(options.keys(), key=lambda x:abs(x-val))]
#        return key + 2

	key = 0
	for t,k in sorted(options.iteritems()):
		if t > val:
			key = k
			break
	print "Mind. "+str(val)+"s warten, wähle Zeitkonstante: "+str(k)+" ("+str(t)+")"
...
#===========================================================================================================
# by CSI
    def SetSensitivityLIA(self, timeconstant = None, vals=3, parallel=False, tlimit=0.95):
        """
        ...
        while bKorrekterBereich == 0:
            ...
            if not parallel:
            	sleeptime = (2 * timeconstant)
	
#            	if debug:
            	print "...sleeping %0.0f s..." %sleeptime
            	time.sleep(sleeptime);

            # Query property value(s).
            iSensitivityLIA = int(self.getSens()); 	# get the set sensitivity
            vValue = senslist[iSensitivityLIA]			# !!!!!!!!

            Rl=[]
            for i in range(vals):  
                 time.sleep(2*timeconstant)
                 Rl.append(self.getR())              
            R = np.max(Rl)               
            ...
            if debug:
        		print Rl
#        		print "max: " + str(R)
        		print("iSensitivityLIA: " + str(iSensitivityLIA) + "|   Range: " + str(vValue))
		...
		if iSensitivityLIA < 26:	
                	iSensitivityLIA += 1
		else:				# Max erreicht, kann nix machen
			bKorrekterBereich = 1

            elif R < (vValue * 0.6):
                newsens = 99
                for key,val in sorted(senslist.iteritems())[::-1]:
                    if R < (val * 0.8):
				newsens = key
                if iSensitivityLIA == newsens:
                    bKorrekterBereich = 1
                else:
                    iSensitivityLIA = newsens

            else:
                bKorrekterBereich = 1

        	#------------------------
            if bKorrekterBereich == 0:
                if iSensitivityLIA < 0:
                    iSensitivityLIA = 0;
                elif iSensitivityLIA > 26 :
                    iSensitivityLIA = 26;
...
#        if debug: 
#            print str(vValue)
        return vValue, bKorrekterBereich
#===========================================================================================================
    ...
    def SerialPollDiagnostic(self):
        ...
        if (not ok):
            ...
            if SPBbit2:
                ...
                ERRSbyte = self.get('ERRS?')# may be subroutine call required
                print 'error-status byte: ', ERRSbyte
            if SPBbit3:
                print 'unmasked bit in LIA status byte set'
                LIASbyte  = self.get('LIAS?') # may be subroutine call required
                print 'LIA-status byte: ', LIASbyte
            if SPBbit4:
                self.write('REST') # not shure that this will help
            if SPBbit5:
                ESRbyte = self.get('*ESR?') # may be subroutine call required
                print 'standard event-status byte: ', ESRbyte
            if SPBbit6:
                SPEbyte = self.get('*SRE?') # may be subroutine call required
...
#reference settings
    def SetRefRms(self,rms):
        ...
        resp = self.set('SLVL', rms)
        return resp
        
#===========================================================================================================
# by CSI
    def SetRefRms2(self,rms):
        """
        The SLVL x command sets the amplitude of the sine output.
        The parameter x is a voltage (real number of Volts). The value of x will
        be rounded to 0.002V. The value of x is limited to 0.004 ≤ x ≤ 5.000[1].
        """
        if rms < 0.004 or rms > 5.0:
            print "WARNING: SetRefRms: parameter rms can only be set to values from 0.004 to 5.0"
...
#input and filter
    ...
    def GetDisplay(self, channel = 1):
        ...
        with no ratio[1].
        """
        resp = self.get('DDEF? ' + str(channel));
        [j,ratio] = resp.rsplit(',')
        return [j,ratio]
...
#===========================================================================================================
# by CSI
    def SNAP2(self,Params=[]):
    	if len(Params) < 2:
            raise Exception("2 or more parameters needed!")    	
        if len(Params) > 6:
            raise Exception("6 or less parameters needed!")  

        Cmdstr = 'SNAP?' + ','.join(map(str, Params))
        resp = self.get(Cmdstr)
#        print resp
       	
        retvals = [float(x) for x in resp.split(',')]
	return retvals
#===========================================================================================================
    ...
    def GetAuxValue(self, number):
        ...
        OutAux = self.get('OAUX?' + str(number))
